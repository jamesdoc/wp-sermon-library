jQuery(document).ready( function( $ ) {
  ...
  // TODO: Turn this into a better function call rather than ugly duplicated code!
  $('.js-sl_sermon_series_image_button').on('click', function(e){
    e.preventDefault();
    $('.js-sl_error').remove();
    if (file_frame) {
      file_frame.open();
  ...
  function sl_return_image(image_data) {
    if (image_data.type !== 'image') {
      $('<div class="error js-sl_error"><p>Please select an image file</p></div>').insertAfter(".js-sl_taxonomy_image_label");
    } else {
      var series_image_id_field = document.getElementById("js-sl_sermon_taxonomy_image");
      series_image_id_field.value = image_data.id;
      $('.js-sl_sermon_series_image_preview').attr('src', image_data.url);
    }
  }
